...
class Window(tk.Tk):
	...
	def playgame(self):
		...
		self.gamemat.set_player_deck(self.information.display)

		self.udp_btn = tk.Button(
			self, text='HOST/JOIN', command=self.displayUDP)
		self.udp_btn.place(relw=0.06, relh=0.03, relx=0.06, rely=0.97)

		self.udpui = udp.UDPUI(self.gamemat, self.information.display)
		self.udpui.withdraw()
